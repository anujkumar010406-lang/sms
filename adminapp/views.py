...
def adminhome(req):
    try:
         if req.session['adminid']!=None:
             ...
             c_count=Teacher.objects.all().count()
             ...
             return render(req,'adminhome.html',{'adminid':adminid,'stu_count':stu_count,'tcount':tcount,'c_count':c_count,'c_count':c_count,'ps':ps,'abs':abs})
    ...
    try:
        if req.session['adminid']!=None:
            ...
            if req.method=="POST":
                ...
                created_date=timezone.now()
                sb=Subject(subject_name=subject_name,sclass=sclass,book=book,steacher=steacher,created_date=created_date) 
                sb.save()
...
def viewsubject(req):
    try:
         if req.session['adminid']!=None:
             ...
             return render(req,'viewsubject.html',{'adminid':adminid,'sb':sb})
...
def editclass(request,id):
    try:
         if request.session['adminid']!=None:
             ...
             return render(request,'editclass.html',{'adminid':adminid,'cl':cl})
    except KeyError:
        return redirect('login')
    ...
    try:
        if req.session['adminid']!=None:
            ...
            if req.method=="POST":
                ...
                created_date=timezone.now()
                th=Teacher(name=name,fname=fname,mname=mname,gender=gender,dob=dob,contactno=contactno,emailaddress=emailaddress,tclass=tclass,address=address,salary=salary,qualification=qualification,password="12345",created_date=created_date) 
                th.save()
...
def viewteacher(req):
    try:
        if req.session['adminid']!=None:
            ...
            return render(req,'viewteacher.html',{'adminid':adminid,'th':th})
            ...
            if req.method=='POST':
                ...
                if stu is not None:
                    return render(req,'addstudent.html',{'adminid':adminid,'cl':cl,'msg':'This emailaddress is already taken'})
                     ...
                     st=Student(rollno=rollno,name=name,fname=fname,mname=mname,gender=gender,dob=dob,contactno=contactno,emailaddress=emailaddress,sclass=sclass,address=address,feespaid=feespaid,duefees=duefees,created_date=created_date,password=password)
                st.save()
...
def viewnoti(req):
    try:
         if req.session['adminid']!=None:
             adminid=req.session['adminid']
             return render(req,'viewnoti.html',{'adminid':adminid,})
